...
from flcore.servers.serverRDA import RDA
...
emb_dim=32
# 替换 BN 为 GN 的函数
def replace_bn_with_gn(model, num_groups=16, device="cuda"):
    for name, module in model.named_children():
        if isinstance(module, nn.BatchNorm2d):
            num_channels = module.num_features
            # 创建 GroupNorm 层并移动到目标设备
            gn_layer = nn.GroupNorm(num_groups=min(num_groups, num_channels), 
                                    num_channels=num_channels).to(device)
            setattr(model, name, gn_layer)
        else:
            replace_bn_with_gn(module, num_groups, device)  # 递归替换
    return model
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "mlr": # convex
            ...
            model = models.resnet18(pretrained=False, num_classes=args.num_classes).to(args.device)
            # 替换模型中的 BN 为 GN
            args.model = replace_bn_with_gn(model, num_groups=16)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-algo', "--algorithm", type=str, default="RDA")
